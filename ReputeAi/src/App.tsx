...
const DEMO_REVIEWS: Review[] = [
  {
    ...
    author: "דני כהן",
    rating: 5,
    text: "שירות מעולה! הגעתי אחרי המלצה ולא התאכזבתי. הצוות מקצועי ואדיב מאוד.",
    source: "Google",
    date: "לפני יומיים",
    sentiment: "positive",
    replied: false,
  ...
  },
  {
    ...
    author: "מיכל לוי",
    rating: 2,
    text: "המתנה ארוכה מדי, לא קיבלתי עדכון על העיכוב. מאכזב מאוד.",
    source: "Google",
    date: "לפני 3 ימים",
    sentiment: "negative",
    replied: false,
...
const AI_FALLBACKS: Record<string, string> = {
  positive:
    "תודה רבה על הביקורת החמה שלך! אנו שמחים שנהנית מהביקור ומצפים לראותך שוב בקרוב 😊",
  negative:
    "אנו מצטערים מאוד על החוויה הפחות טובה. אשמח ליצור איתך קשר אישי לפתרון מיידי של הנושא.",
...
const genChallenge = async (v: string) => {
...
const Stars = ({ n, size = 13 }: { n: number; size?: number }) => (
  <span style={{ fontSize: size, letterSpacing: 1 }}>
    {[1, 2, 3, 4, 5].map((i) => (
      <span key={i} style={{ color: i <= n ? "#fbbf24" : "#2a2a3e" }}>
        ★
      </span>
    ))}
  </span>
...
export default function App() {
  const [page, setPage] = useState<
    "landing" | "dashboard" | "stats" | "onboarding"
  >("landing");
 ...
 useEffect(() => {
  ...
  const getValidToken = async (): Promise<string | null> => {
    ...
    try {
      ...
      localStorage.setItem(
        "google_token_expiry",
        String(Date.now() + (t.expires_in - 60) * 1000)
      );
      setGoogleUser((prev) =>
        prev ? { ...prev, accessToken: t.access_token } : null
      );
  ...
  const fetchBusinessAccounts = async (token: string) => {
    ...
    try {
      ...
      if (res.status === 429) {
        showNotif("Google API עמוס – נסה שוב בעוד דקה", "#fbbf24", "⏳");
        return;
      }
      ...
      showNotif("לא ניתן לטעון חשבונות עסקיים", "#f87171", "✕");
    }
  };
  ...
  const fetchLocations = async (token: string, aid: string) => {
    try {
      ...
      if (res.status === 429) {
        showNotif("Google API עמוס", "#fbbf24", "⏳");
        return;
      }
      const data = await res.json();
      ...
      showNotif("לא ניתן לטעון מיקומים", "#f87171", "✕");
    }
  };
  ...
  const fetchRealReviews = useCallback(async () => {
    ...
    try {
      ...
      if (!token) {
        showNotif("נא להתחבר מחדש", "#f87171", "✕");
      ...
      if (raw.length === 0) {
        setReviews([]);
        showNotif("אין ביקורות עדיין", "#fbbf24", "⚠");
        setLoadingReviews(false);
        return;
      }
      ...
      showNotif("שגיאה בטעינת ביקורות", "#f87171", "✕");
    }
    setLoadingReviews(false);
  ...
  const submitReply = () => {
    ...
    setReviews((prev) =>
      prev.map((r) => (r.id === activeReview.id ? { ...r, replied: true } : r))
    );
  ...
  const submitLead = async () => {
    if (!leadForm.name || !leadForm.phone) {
      showNotif("נא למלא שם וטלפון", "#fbbf24", "⚠");
      return;
    }
    try {
      await fetch("https://formspree.io/f/mpqyqoed", {
        ...
        body: JSON.stringify({ ...leadForm, source: "landing_bar" }),
      });
    } catch {
      /* ignore */
    }
  ...
  const submitAnalysis = async () => {
    if (!analysisForm.email) {
      showNotif("נא להזין אימייל", "#fbbf24", "⚠");
      return;
    }
    try {
      await fetch("https://formspree.io/f/mpqyqoed", {
                ...
                <div key={i} style={{ flex: 1 }}>
                  ...
                  <div
                    style={{
                      fontSize: 11,
                      color:
                        i <= onboardingStep ? "var(--accent2)" : "var(--sub2)",
                      marginTop: 6,
                      fontWeight: 600,
                    }}
                  >
      ...
      >
        ...
        <div style={{ maxWidth: 1100, margin: "0 auto", padding: "32px 24px" }}>
          ...
          {isDemoMode && (
            ...
            >
              <div style={{ color: "#fbbf24", fontSize: 14, fontWeight: 600 }}>
                🎭 מצב דמו — מציג נתונים לדוגמה
              </div>
              <GoogleBtn
                onClick={handleGoogleLogin}
                text="חבר את העסק האמיתי שלך"
              />
            </div>
          )}
          ...
          {loadingReviews ? (
            <div style={{ textAlign: "center", padding: 80 }}>
              ...
              <div
                style={{ fontSize: 18, fontWeight: 700, color: "var(--sub)" }}
              >
                טוען ביקורות...
              </div>
            </div>
            ...
            <>
              ...
              >
                <div style={{ display: "flex", gap: 6 }}>
                  {[
                    { key: "all", label: "הכל" },
                    { key: "positive", label: "⭐ חיוביות" },
                    { key: "negative", label: "⚠ שליליות" },
                    { key: "neutral", label: "• ניטרליות" },
                  ].map((f) => (
                    <button
                      key={f.key}
                      ...
                      style={{
                        ...
                        color:
                          filter === f.key ? "var(--accent2)" : "var(--sub)",
                      }}
                    >
                      {f.label}
                    </button>
                      ...
                      >
                        <div style={{ display: "flex", gap: 14, flex: 1 }}>
                          ...
                          <div style={{ flex: 1 }}>
                            ...
                            <div style={{ color: "var(--sub2)", fontSize: 12 }}>
                              {review.date}
                            </div>
                          </div>
                        </div>
        ...
        {activeReview && (
          <Modal onClose={() => setActiveReview(null)}>
            ...
            >
              ...
              >
                <Avatar
                  letter={activeReview.avatar || activeReview.author[0]}
                  size={32}
                />
        ...
        {whatsappModal && (
          <Modal onClose={() => setWhatsappModal(false)}>
            ...
            <div
              style={{ color: "var(--sub)", fontSize: 14, marginBottom: 16 }}
            >
            ...
            >
              ...
              >
                היי! 😊 תודה שבחרת ב{onboardingData.businessName || "העסק שלנו"}
                .<br />
                נשמח אם תוכל לשתף את חוויתך – ביקורת קצרה עוזרת לנו מאוד!
                <br />
                👉 [קישור לביקורת בגוגל]
              </p>
            </div>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              ...
              <button style={btnGhost} onClick={() => setWhatsappModal(false)}>
                ביטול
              </button>
            </div>
          </Modal>
    ...
    >
      ...
      >
        <div
          style={{
            ...
            borderRadius: "50%",
            background:
              "radial-gradient(circle, rgba(124,110,245,0.08) 0%, transparent 70%)",
            animation: "orb 20s ease infinite",
          }}
        />
        <div
          style={{
            ...
            animation: "orb 25s ease infinite reverse",
          }}
        />
        <div
          style={{
        ...
        <div
          style={{
            ...
            inset: 0,
            backgroundImage:
              "linear-gradient(rgba(124,110,245,0.03) 1px, transparent 1px), linear-gradient(90deg, rgba(124,110,245,0.03) 1px, transparent 1px)",
      ...
      >
        ...
        >
          ...
          <span
            style={{
              background: "linear-gradient(135deg, #7c6ef5, #a78bfa, #e879f9)",
        ...
        <div
          ...
          style={{ marginTop: 80, animation: "float 6s ease infinite" }}
        >
          <DashboardPreview />
        </div>
      </section>
      ...
      >
        <div style={{ textAlign: "center", marginBottom: 64 }}>
          ...
          <p style={{ color: "var(--sub)", fontSize: 16 }}>
        ...
        >
          {[
            ...
            <div key={i} style={{ textAlign: "center" }}>
              ...
              <div
                style={{ color: "var(--sub)", fontSize: 14, lineHeight: 1.7 }}
              >
                {step.desc}
              </div>
            </div>
        ...
        >
          {[
            ...
            >
              ...
              <div
                style={{ color: "var(--sub)", fontSize: 14, lineHeight: 1.6 }}
              >
                {f.desc}
              </div>
            </div>
      ...
      >
        <div
          style={{
            background:
              "linear-gradient(135deg, rgba(124,110,245,0.12), rgba(167,139,250,0.08))",
        ...
        >
          <div
            style={{
              ...
              inset: 0,
              background:
                "radial-gradient(circle at 50% 0%, rgba(124,110,245,0.15), transparent 60%)",
              pointerEvents: "none",
            }}
          />
          <h2
          ...
          {[
            ...
            <div
              ...
              style={{
                background: "var(--bg2)",
                border: `1px solid ${
                  (plan as any).popular ? "var(--accent)" : "var(--border)"
                }`,
                borderRadius: "var(--radius)",
                padding: "32px 28px",
                position: "relative",
      ...
      >
        {leadSent ? (
          ...
          <>
            ...
            {[
              { ph: "שם", key: "name", w: "120px" },
              { ph: "טלפון", key: "phone", w: "130px" },
              { ph: "אימייל", key: "email", w: "160px" },
            ].map(({ ph, key, w }) => (
              <input
                key={key}
                placeholder={ph}
                ...
                style={{ ...inputStyle, width: w, direction: "rtl" }}
                onFocus={(e) => (e.target.style.borderColor = "var(--accent)")}
                onBlur={(e) => (e.target.style.borderColor = "var(--border2)")}
              />
            ))}
            <button
              style={{ ...btnPrimary, padding: "11px 24px" }}
              onClick={submitLead}
            >
              שלח ←
            </button>
        ...
        >
          {analysisSubmitted ? (
            <div style={{ textAlign: "center", padding: "40px 20px" }}>
              ...
              >
                אנחנו מנתחים את הביקורות של העסק שלך. תקבל דוח מפורט תוך 24 שעות
                לאימייל.
              </p>
              <button
                style={btnPrimary}
            ...
            <>
                ...
                {[1, 2].map((i) => (
                  <div
                    ...
                    style={{
                      ...
                      borderRadius: 3,
                      background:
                        i <= analysisStep ? "var(--accent)" : "var(--border2)",
                      transition: "background 0.3s",
                    }}
                  />
                ))}
              ...
              {analysisStep === 1 && (
                ...
                >
                  <GoogleBtn
                    onClick={handleGoogleLogin}
                    text="חבר Google Business — תוצאות אוטומטיות"
                  />
                  <div
                    style={{
                      textAlign: "center",
              ...
              {analysisStep === 2 && (
                ...
                >
                  ...
                  <div style={{ display: "flex", gap: 10, marginTop: 4 }}>
                    <button
                      style={{ ...btnGhost, padding: "12px 20px" }}
...
const Nav = ({
  ...
  <nav
    style={{
      ...
      padding: "16px 32px",
      borderBottom:
        scrollY > 60 ? "1px solid var(--border)" : "1px solid transparent",
      background: scrollY > 60 ? "rgba(5,5,10,0.95)" : "transparent",
      ...
      <div style={{ display: "flex", gap: 28, alignItems: "center" }}>
        {[
          ...
          <a
            ...
            onMouseLeave={(e) =>
              ((e.target as HTMLElement).style.color = "var(--sub)")
            }
          >
            {label}
          </a>
    ...
    <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
      ...
      {page === "dashboard" || page === "stats" ? (
        <>
          <div
            style={{
              background: isDemoMode
                ...
                : "rgba(34,211,160,0.1)",
              border: `1px solid ${
                isDemoMode ? "rgba(251,191,36,0.3)" : "rgba(34,211,160,0.3)"
              }`,
              borderRadius: 100,
              padding: "4px 12px",
              fontSize: 12,
              color: isDemoMode ? "#fbbf24" : "#22d3a0",
              fontWeight: 700,
            }}
          >
            {isDemoMode ? "🎭 דמו" : "✓ חי"}
          ...
          <button
            style={{ ...btnGhost, padding: "8px 14px", fontSize: 13 }}
        ...
        <>
          <button
            style={{ ...btnGhost, padding: "9px 18px", fontSize: 14 }}
            onClick={onDemo}
          >
            כניסה לדמו
          </button>
...
const DashboardPreview = () => (
  <div
    style={{
      ...
      maxWidth: 700,
      margin: "0 auto",
      boxShadow: "0 40px 80px rgba(0,0,0,0.6), 0 0 60px rgba(124,110,245,0.1)",
      textAlign: "right",
    }}
  >
    {/* mini header */}
